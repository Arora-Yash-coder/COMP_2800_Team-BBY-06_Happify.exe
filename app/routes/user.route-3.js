module.exports = function (app) {
	...
	app.post('/daily_tasks', (req, res) => {
		let daily_tasks = req.body
		console.log("daily_tasks-=============req.body")
	...
	app.get('/daily_tasks', function (req, res) {
		...
		MongoClient.connect(dbConfig.url, function (err, db) {
			...
			let find_user_get_ready = dbo.collection("users").findOne({
				_id: ObjectId(req.session.user_sid)
			}).then(result => {
				try {
					//if the user exists
			...
			let find_task = dbo.collection("daily_tasks").find({
				"id": { $nin: go_ahead }
			...
			let reject = console.log("no")

			const promiseA = new Promise(find_user_get_ready,reject)
			promiseA.then()
		// var added_item = []
		// //find those that's not in the done list
		// let find_not_done = dbo.collection("daily_tasks").find({
		// 	"id" :{$nin: go_ahead}
		// }).limit(n_to_go).toArray( (err, daily_tasks) => { 

		// 	console.log("go_ahead in 400")
		// 	console.log(go_ahead)
		// 	console.log("n_to_go")
		// 	console.log(n_to_go)
		// 	console.log("daily_tasks")
		...
		console.log("doing update now ")
		let update_archived = dbo.collection("users").updateOne(
		...
		console.log("doing finding and rendering now ")
		console.log("added_item in 436")
			console.log(added_item)

		let find_and_render = dbo.collection("daily_tasks").find({
			"id" :{$in: added_item}
		}).limit(n_to_go).toArray( (err, daily_tasks) => { 
		...
		// find_user_get_ready.then(find_not_done).then(update_archived).then(find_and_render)
